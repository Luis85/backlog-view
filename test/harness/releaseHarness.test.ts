...
describe('the release harness mounts', () => {
	...
	it('draws every release in the index, in the order the index sorts them', () => {
		...
		// Dated first by date, undated last — the fixture exists to show that order, so a
		// fixture that stopped showing it would leave the screenshot proving nothing.
		expect(names(containerEl)).toEqual(['0.8', '0.9', '1.0', 'Someday']);
	});
	...
	it('mounts the three unconfigured states each variant exists for', () => {
		...
		// all seven the fixture holds, not just the two the index reports — so the note under
		// the empty state is the whole of what it can report.
		expect(empty.querySelector('.pbl-rel-note')?.textContent).toContain('7 items');
