...
describe('toolbar backfill', () => {
	...
	it('claims no items at all when the first note stops the batch', async () => {
		const vault = new FakeVault();
		vault.addFile('Epic.md', { frontmatter: { type: 'Epic', order: 10 } });
		vault.addFile('First.md', { frontmatter: { order: 20 }, parentLink: 'Epic' });
		vault.addFile('Second.md', { frontmatter: { order: 30 }, parentLink: 'Epic' });
		const { containerEl } = makeView(vault, noOptionalProperties());
		vault.onNextProcess('First.md', (fm) => {
			fm['type'] = 'Resource';
		});
		...
		// The writer's refusal is the only thing said: "updated 0 items" would be a second
		// sentence about the same nothing, and `init.nothingToDo` would be untrue.
		expect(Notice.messages.some((m) => m.includes('updated'))).toBe(false);
		expect(Notice.messages.some((m) => m.includes('already have the properties'))).toBe(false);
	});
