...
describe('what an edit is, and is not', () => {
	...
	it('says so and offers no edit where either key holds something unreadable', () => {
		...
		// saying it on 2026-09-01 when ✨ began binding one. The last is the capacity
		// comparison, added by Task 4: `RELEASE_CONFIG` binds both `capacityProperty` and
		// `capacityUnit` (✨ binds the unit too, since the product owner's reversal), and this
		// fixture's `R.md` declares no `capacity` — so only the capacity half is named
		// ("declares no capacity"); the unit is bound and says nothing. Listed rather than
		// filtered out, so the exact list still says no OTHER field went quietly unreadable.
		expect(said).toEqual([
			'Status unreadable',
