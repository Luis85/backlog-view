...
- **`npm run perf` runs that page headlessly and prints the table**, for a session with no
  ...
  **That list is exhaustive and anything else is refused** — a misspelling used to be
  stored and ignored, so the run printed a plausible table for a command nobody meant.
