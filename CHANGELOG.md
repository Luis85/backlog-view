...
- **A resource's row no longer has a New button.** Creating from one wrote the resource
  onto the note and then left it on the shelf, because creation supplies no date and a row
  needs one to place a bar — so a click on a specific row produced a card somewhere else.
  Create from the toolbar and assign it, or drag it into a row once it has dates.
...
- **Fold a resource's row, and fold work inside it.** Each row on the resources axis has a
  disclosure that shuts everything under that person — their bars, their absences, the
  notes their row places — leaving the header, so a roster longer than the pane stops being
  a scroll. Inside an open row, a bar folds its own children exactly as it does on the plain
  timeline, and **only the ones sitting in that same row**: a chevron can never hide
  somebody else's work. Both survive closing the view, and **Clicking a row folds it** is
  now offered in the toolbar here, having been withheld while nothing folded.

- **Edit an absence you have already placed.** **Edit absence** joins Delete on a stretch's
  own context menu and reopens the same form you added it with, filled in. Changing who it
  is for or the days it covers rewrites the note; changing the title renames it, so links
  to it follow.

- **Resources you name are offered everywhere, not only on the roadmap.** Names typed into
  **Resources (in order)** now lead the **Set assignee** list on every row and card, ahead
  of the names your notes already carry. The roster stays optional and never narrows
  anything: an assignee a note names is still offered whether or not the roster mentions
  them, and a name in neither is still one **New assignee...** away.

- **Assigning someone adds them to the roster.** Picking or typing a name that
  **Resources (in order)** does not list appends it, so the row they get is a declared one
  instead of a stray marked "not one of the declared resources". Clearing an assignee takes
  nobody off — the roster is a list of who exists, not of who is busy.

### Fixed

- **An inferred bar is no longer draggable on the resources axis.** A bar whose dates come
  from its children has no date of its own to move from, so the timeline has always refused
  to let you drag one — and for one build the resources axis let you, which made the same
  bar behave differently on two views of the same plan. Use **Set assignee** or
  **Alt+Up/Down** to move it between rows.

- **The grid no longer shows through the resource name column.** On a scrolled resources
  axis the today line and the day shading were visible straight through the sticky column
  of names — worst on absence rows, where the red line ran through the title, and as a red
  tick at the foot of every band header.
...
- **An absence stretch is a drop target like the rest of its row.** Dropping work onto the
  ...
  though every other line of that row assigns to it. Since absences draw at the top of a
  row, that dead strip sat directly between a row's title and its work.

- **An absence far outside the drawn window no longer paints on the wrong days.** The grid
  sized itself from the work in it and never from the absences, so a stretch beyond that
  range was squeezed onto the last visible day as a one-day block — worst for a row that
  exists only because someone is away, where nothing else in the row had any say in the
  dates on screen. The grid now stretches to include every absence it draws.
