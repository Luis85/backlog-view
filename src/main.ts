...
import { registerMyWorkView } from './view/mywork/register';
...
export default class ProductBacklogPlugin extends Plugin {
	onload(): void {
		...
		registerReleaseView(this, lock);
		registerMyWorkView(this, lock);
		...
		// which base it belongs to: three stored picks are NOTE paths (the board's iteration
		// scope, the release view's open release and the my-work view's picked person), and
		// a stale one resolves to nothing. For the release and my-work views that is worse
		// than a lost position — the view falls back to its index, or to no pick, without a
		// word, so a renamed note reads exactly like a deleted one. Here rather than on a
		// view for `rekeyBase`'s own reason: the pick belongs to a saved view, not to
		// whichever view happens to be loaded, and normally only one such view is on screen.
		this.registerEvent(
