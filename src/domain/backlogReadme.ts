...
/** One state the view offers, and whether the configuration declares it. */
export interface StateEntry {
	value: string;
	/** False when the value was observed in the notes rather than configured. */
	observed: boolean;
}
...
export function readmeStates(settings: BacklogSettings, observedStates: string[]): StateEntry[] {
	...
	const entries = offered.map((value) => ({ value, observed: !declared }));
	...
	for (const value of observedStates) {
		...
		entries.push({ value, observed: true });
	}
	return entries;
}
...
function stateSection(settings: BacklogSettings, states: StateEntry[]): string[] {
	...
	const rows = states.map(
		(s) =>
			`| ${code(s.value)} | ${done.has(s.value.toLowerCase()) ? 'Yes' : 'No'} | ${s.observed ? 'Observed in these notes' : 'Declared in the view'} |`,
	);
	return [
		'## Workflow states',
		'',
...
function exampleSection(settings: BacklogSettings, states: StateEntry[]): string[] {
	...
	if (settings.stateKey && states.length > 0) lines.push(`${settings.stateKey}: ${states[0].value}`);
