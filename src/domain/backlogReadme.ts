 ...
 *
 * `source` is user data (a base path, a view name), and an HTML comment cannot hold
 * `--` or `>`, so both are flattened rather than escaped: the line is an identity to
 * compare, not a path to resolve.
 */
export function readmeMarker(source: string): string {
	const safe = source.replace(/-{2,}/g, '-').replace(/[<>]/g, '');
	return `${README_MARKER_PREFIX} from "${safe}". Rewritten in full whenever it is regenerated. -->`;
}
