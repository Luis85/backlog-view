 ...
 */
export function rowHidden(item: BacklogItem, filter: FilterState, settings: BacklogSettings, applyFilter: boolean): boolean {
	...
	if (filter.active) {
		if (applyFilter && !filter.keeps(item.file.path)) return true;
	...
	if (item.outsideFilter) return !item.children.some((child) => !rowHidden(child, filter, settings, applyFilter));
	return false;
}
