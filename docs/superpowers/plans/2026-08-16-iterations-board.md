...
**Both actions run the `configProblems` gate first**, before the dialog opens. `createBacklogItem` performs no validation of its own — only `promptCreateItem` does — so calling it directly from a toolbar action is a creation surface that accepts a configuration every other one refuses. With the goal property colliding with the type key, the goal would overwrite `type: Iteration` and the new note would not be an iteration at all. `runInit` is the precedent and the reason: it gates itself before touching options or notes, because an action that changed things and then had its writes refused leaves the view worse than it found it. Gate on open rather than on submit, so the reader is told before typing a name and two dates.

Creating then goes through `createBacklogItem` with the type, the folder, both dates and the goal in **one** write, and opens the note.
