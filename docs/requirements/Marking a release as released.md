...
| **Preconditions** | The release status key, its released values and the actual-date key are configured |
...
3. One batch writes the released status value and today's date, into the two keys this view
   names.
...
- **3a — the status key or its released values are unconfigured.** The action is not offered at
  all, and the release screen says which option to bind. A key with no value list is
  unconfigured, not empty.
...
- With the status key, its released values, or the actual-date key unconfigured, the action is
  absent and the missing option is named.
...
`src/storage/frontmatter.ts` over `src/view/writeGate.ts`. The status key, its released values
and the actual-date key are declared in `src/domain/viewOptions.ts`, with the same-key refusal
in `src/domain/settingsConsistency.ts`. The confirmation reuses `src/ui/prompts.ts`, and the
