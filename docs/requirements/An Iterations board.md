...
rule about which board uses which, and the user chose one door — the `Boards` button is
...
exactly as an iteration scope is — `Boards` reopens the board this view was last on — as
