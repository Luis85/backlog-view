...
- **The consolidated value is written back; nothing else is.** The business value is the
  ...
  **A written total is a copy, and a copy can be wrong.** That cost is accepted here
  ...
  one reads, its range, its direction, **the model's own output range**, the formula that combines them
  into the written value — an
  indicator's formula persists nothing and is stamped nowhere — **and the rubric
