...
- The lead column's resize grip, which is where this feature keeps everything jsdom
  ...
  checked.**
- The five state-colour slots under a real theme, light and dark: each reads as distinct
  from the others and from the red, cyan and green that already mean today, a milestone
  and done; a done bar is unmistakably green whatever slot its own state occupies; and a
  swatch and its bars match at a glance rather than only by variable name. The
  stylesheet check behind this says which variable each rule names and nothing about
  what those resolve to, and per ADR 0020 the harness settles layout and not colour.
  **Never checked.**
