...
- **Writes are silent and instant.** A drag rewrites the dragged item's `parent` and
  `order` with no dialog and no save step, and one gesture is not one note: a backfill
  that ranks sixty of them is a single change too. Undo exists precisely because of that,
  and undo is a toolbar arrow with a tooltip.
