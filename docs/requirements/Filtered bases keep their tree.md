...
| A ranking peer | Never **written to** — though its `order` is still **read**, so nothing lands above something visible, and a RANKED one is a legal anchor to be dropped beside at the focus level ([ADR 0032](../adrs/0033-order-is-a-global-rank.md)). An **unranked** one is dropped from every peer population instead — never a peer, never an anchor — so a child drawn beneath one cannot outdent past it either: the command is withheld from its menu, and Alt+Left reports rather than writing a rank that lands before the row it named. |
