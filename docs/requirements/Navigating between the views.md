...
An item selected in one view can be opened where something else has more to say about it —
its note, or a base whose saved view covers that capability — and where no such view exists
in the vault nothing is offered, so this never advertises a capability nobody added.
...
**Outcome** — The views feel like one product without any of them depending on another, and
the connection is a link rather than a wire.
