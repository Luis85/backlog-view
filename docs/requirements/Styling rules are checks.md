...
| The clear control's containing block is the ROW that reserves nothing (`.pbl-est-dim`), never the head that holds the gutter open (`.pbl-est-dim-head`), and that gutter is not narrower than the control it reserves for | **Checked** 2026-08-21, `test/view/estimation/styleRules.test.ts`, both halves in both directions — the `position` present on the row and absent from the head, the new token present and the old 20px one gone. Watched failing. An absolute inset resolves against its containing block's PADDING box, so `position: relative` on the element carrying `padding-inline-end` measured `inset-inline-end: 0` from INSIDE the reservation and drew the control over the last point (reported from a vault). The width half is arithmetic the test pins rather than measures: 32px is `.clickable-icon`'s 4px/6px padding around an `--icon-m` glyph, 18px on the desktop root and 20px at the touch breakpoint. What no rule check reaches is whether the control lands inside the gutter, or what a theme setting its own `--icon-size` does to it — a live-vault look, and it stays owed |
| A row the keyboard steps to reserves the sticky header's own height as `scroll-margin-block-start` (`.pbl-est-row`) | **Checked** 2026-08-21, same file — the declaration's presence on the row, and nothing more. Watched failing. `block: 'nearest'` scrolls a row flush to the scroller's edge, which is under `.pbl-est-head`, so an upward step parked the selection behind the column labels; 32px is that header's own `min-height`. Two things beside it are UNCHECKED and stated rather than implied: the panel's matching absence is deliberate (`renderPanel` restores the scroll position, so a refocused point button is already on screen, and `.pbl-est-header`'s height is content-driven) and lives in the stylesheet's comment with no assertion under it, and whether the offset is ENOUGH is a measurement jsdom cannot make — a theme that moves the row height or the font moves it |
