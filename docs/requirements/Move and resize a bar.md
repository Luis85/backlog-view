...
- **1a — the bar has one date.** The body drag moves the date it has and the open end
  ...
  the open end is exactly how the missing date gets written.
...
- **1c — the bar is inferred.** No gesture holds: an inferred span is display
  ([[Spans roll up the tree]]), and dragging it would write an inference. The path to a
  real plan on a parent is stating real dates — the menu offers it on any row
...
- One-dated bars keep their open end on body drags, and dragging the open end writes
  the missing date; ends clamp at equal and never cross.
- Inferred bars take no gesture; a dated parent's bar moves only the parent.
