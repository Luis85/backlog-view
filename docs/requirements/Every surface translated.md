...
| `releaseOptions.ts`'s option headings | **Text**, for that same reason — the third instance of that literal, one per registered view. It arrived after this slice, with the release view, and was written keyed rather than swept, so it joined `SWEPT` on arrival: a sibling left outside the ban would have been an accident of arrival date and not a decision. |
...
| `backlogReadme.ts`, `readmeStamps.ts` and `readmePlanning.ts` | **The `view/manual/` question, second, third and fourth instance.** All three write authored PROSE, and one of them is what writes it INTO the vault. Stated below rather than decided here. `readmePlanning.ts` arrived after this slice, with the release view; it is `backlogReadme.ts`'s own planning section split out at the 400-line cap, so it inherits the verdict rather than raising a new question. |
