...
3. Below them, one term per answered dimension: label, score, weight.
...
- The confidence-adjusted value and the value-to-effort indicator each render only once
  their own inputs exist, each under its own label.
...
- The panel's scroll position survives a rebuild on the same item, starts at the top for a
  different item, and is clamped when the rebuilt panel is shorter.
...
only once its own inputs exist) · `src/domain/weightedScore.ts` (`computeTotal`, whose
result and coverage the summary line reports, and `round2`) ·
`src/domain/estimationItems.ts` (`EstimationItem`, the answers the terms are read from).
