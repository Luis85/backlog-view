...
the same backlog — the backlog/board toggle, columns from the workflow the view options
define, card moves as gated state writes, and the hierarchy showing through on the
board. Nothing under it is built; every note states the precedent or the codebase seam it
rests on, from a survey of the Kanban Guide, the major trackers and the Obsidian
ecosystem run on 2026-08-01.

Its use cases are the argument for writing a PBI as a use case *before* building it
rather than after. Their `Where it lives` sections say **nothing yet** and then name the
module the work will extend, which is a design claim a reader can disagree with; their
