...
- **400-line max per file in `src/`**, 450 in `test/`, enforced by lint. `src/domain/model.ts` is already 611 lines — it is over the cap under an existing exemption; do not grow it more than this plan requires, and do not add a second concern to it.
