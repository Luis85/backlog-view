 ...
 * `Test suite`, an `Iteration` and a `Resource` are roots by NATURE: a release date, a
 * time box and a person are all owned by the plan rather than by an epic, and a suite
 * hangs from nothing because the tests are their own list rather than a branch of the
 * plan. The suite is the first root by nature that has CHILDREN, which is what makes
 * these two questions rather than one.
 */
const ROOT_TYPES = new Set(["Epic", "Milestone", "Test suite", "Iteration", "Resource"]);
