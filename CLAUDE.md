...
other's view cannot read" is data. 577 keys are in it (counted two ways on 2026-08-22 and
...
carry their value on the FOLLOWING line, which is what such an instrument gets wrong). Two
branches each counted their own catalog on the same day — 550 and 542 — and this is the
merged one, which is neither.
